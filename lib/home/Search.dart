...
class Search extends StatefulWidget {
  ...
  const Search({Key? key, required this.colorPrimary}) : super(key: key);
...
class _SearchState extends State<Search> {
    ...
    return Scaffold(
      ...
      bottomNavigationBar: Container(
        ...
        child: ElevatedButton(
            ...
            child: load ?
            Text("See all ${properties.isNotEmpty ? "${properties.length} " : ""}Properties", style: TextStyle(color: MyColors.white),)
  ...
  getPropertyDesign(List<Category> categories) {
    return ListView.separated(
      ...
      itemBuilder: (BuildContext buildContext, int index) {
        return getPropertyTab(categories[index], index, categories.length);
      },
      separatorBuilder: (BuildContext buildContext, int index) {
        return const SizedBox(
          height: 10,
  ...
  getPropertyTab(Category category, int ind, int total) {
    bool select = false;
  ...
  Future<void> saveSearch(String name) async {
    ...
    Response response = await APIService().saveSearch(data);
  ...
  getFilters() {
    ...
    String subAreas = subArea.length>1 ? getSelectedSubAreas() : "";
  ...
  String getSelectedSubAreas() {
    ...
    for (var element in this.subArea.sublist(1)) {
